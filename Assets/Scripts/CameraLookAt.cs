...
public class CameraLookAt : MonoBehaviour
{
    ...
    [SerializeField]
    private TMP_InputField camSpeedInput;
    ...
    void Update()
    {
        camSpeedInput.onValueChanged.AddListener(Value => { camSpeed = float.Parse(Value); Debug.Log($"Cam speed is {camSpeed}"); });
        ...
        if (true) {
            ...
            if (Input.GetKey(KeyCode.I)) { cam.transform.Translate(camSpeed * Vector3.forward * Time.deltaTime); }
            if (Input.GetKey(KeyCode.K)) { cam.transform.Translate(camSpeed * Vector3.back * Time.deltaTime); }
            if (Input.GetKey(KeyCode.J)) { cam.transform.Translate(camSpeed * Vector3.left * Time.deltaTime); }
            if (Input.GetKey(KeyCode.L)) { cam.transform.Translate(camSpeed * Vector3.right * Time.deltaTime); }
            if (Input.GetMouseButton(0)) {
                cam.transform.Rotate(0f, 0.15f * camSpeed * Input.GetAxis("Mouse X"), 0f, Space.World);
                cam.transform.Rotate(0.15f * camSpeed * Input.GetAxis("Mouse Y"), 0f, 0f, Space.World);
                Vector3 rotation = new Vector3(cam.transform.localEulerAngles.x, cam.transform.localEulerAngles.y, 0f);
                cam.transform.rotation = Quaternion.Euler(rotation);
            }
